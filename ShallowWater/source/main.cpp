...
static void Render(Sw_grid *grid) {
    ...
    **************/
    // COLOR
    glm::vec3 water_color_dif(0.1,0.3,0.7);
    gProgram->setUniform("color_dif", water_color_dif);
    glm::vec3 water_color_amb(0.0,0.1,0.05);
    gProgram->setUniform("color_amb", water_color_amb);
     ...
     ****************/
    // COLOR
    glm::vec3 ground_color_dif(0.0,0.0,0.0);
    gProgram->setUniform("color_dif", ground_color_dif);
    glm::vec3 ground_color_amb(0.2,0.2,0.2);
    gProgram->setUniform("color_amb", ground_color_amb);
    ...
    *****************/
    ...
    // COLOR
    glm::vec3 box_color_amb(0.5,0.2,0.2);
    gProgram->setUniform("color_amb", box_color_amb);
...
void Update(float secondsElapsed, bool &bSolving, bool &bRain, bool &bTsunami, bool &bBlob, bool &bBeachRise, bool &bMoveUp, bool &bMoveDown, bool &bMoveNorth, bool &bMoveEast,
			bool &bMoveSouth, bool &bMoveWest) {
...
void AppMain() {
	...
	bool bMoveNorth	= false;
	bool bMoveEast	= false;
	bool bMoveSouth	= false;
	bool bMoveWest	= false;
    ...
    while(glfwGetWindowParam(GLFW_OPENED)){
        ...
        Update(dt, bSolving, bRain, bTsunami, bBlob, bBeachRise, bMoveUp, bMoveDown, bMoveNorth, bMoveEast, bMoveSouth, bMoveWest);
		...
		if(bMoveEast){
			box->x += Vector3f(1.0f, 0.0f, 0.0f)*movementSpeed;
			bMoveEast = false;
		}
		if(bMoveWest){
			box->x += Vector3f(-1.0f, 0.0f, 0.0f)*movementSpeed;
		...
		if(bMoveSouth){
			box->x += Vector3f(0.0f, 1.0f, 0.0f)*movementSpeed;
			bMoveSouth = false;
		}
		if(bMoveNorth){
			box->x += Vector3f(0.0f, -1.0f, 0.0f)*movementSpeed;
